...
def create_model(model_name: str, in_channels: int = 7, out_channels: int = 2, **kwargs):
    ...
    # Import Swin Transformer encoder
    try:
        from .swin_encoder import create_swin_model
        has_swin = True
    except ImportError:
        has_swin = False
    ...
    # Add Swin Transformer models if available
    if has_swin:
        swin_models = {
            "swin_tiny": lambda: create_swin_model("tiny", in_channels=in_channels, out_channels=out_channels, **kwargs),
            "swin_small": lambda: create_swin_model("small", in_channels=in_channels, out_channels=out_channels, **kwargs),
            "swin_base": lambda: create_swin_model("base", in_channels=in_channels, out_channels=out_channels, **kwargs),
        }
...
if __name__ == "__main__":
    ...
    model = ResUNet(in_channels=6, out_channels=6, attention="cbam")
    with torch.no_grad():
        y = model(x)
    ...
    model = ResUNet(in_channels=6, out_channels=6, use_se=False)
    with torch.no_grad():
        y = model(x)
    print(f"  Output: {y.shape}")
    print(f"  Parameters: {sum(p.numel() for p in model.parameters()):,}")
    ...
    model = UNetClassic(in_channels=6, out_channels=6)
    with torch.no_grad():
        y = model(x)
    print(f"  Output: {y.shape}")
    print(f"  Parameters: {sum(p.numel() for p in model.parameters()):,}")
