...
| **Transformer-Tiny** | ✓ | ✗ | ✓ | ✗ | ✓ | ✗ | **22.3M** |
| **Transformer-Small** | ✓ | ✗ | ✓ | ✗ | ✓ | ✗ | ~45M |
| **UNet++** | ✓ | ✗ | ✓ | ✗ | ✓ | ✗ | ~35M |
| **UNet++ (CBAM)** | ✗ | ✓ | ✓ | ✗ | ✓ | ✗ | ~35M |
| **UNet++ Small** | ✓ | ✗ | ✓ | ✗ | ✓ | ✗ | ~9M |
...
# Transformer encoder (CNN-Transformer hybrid with attention)
model = create_model('swin_tiny')
model = create_model('swin_small')
...
Top-down pathway with lateral connections for better multi-scale feature fusion.
Reference: https://arxiv.org/abs/1612.03144
```python
# Using FPN decoder (recommended for multi-scale features)
from src.models.fpn_decoder import ResUNetFPN, create_fpn_model

model = create_fpn_model('resunet_fpn')
model = create_fpn_model('resunet_fpn_cbam')
...
Apply augmentations at inference time and average predictions for better results.
```bash
python -m src.main --mode train --use_tta
```
```python
# Using TTA in code
from src.utils.tta import TestTimeAugmentation, apply_tta

# Wrap model with TTA
tta = TestTimeAugmentation(model, flip=True, rotate=True, scale=True, n_scales=3)
pred = tta.predict(input_tensor)

# Or simple function
pred = apply_tta(model, input_tensor, flip=True, rotate=True, n_scales=3)
```
